...
## Connectivity, Bipartite Graph

- Graph G is called `connected` if there is a path between any two vertices.
- DAG: Directed Acyclic Graph - a directed graph without cycles, which allows for topological sorting.
- An undirected graph G is called a `bipartite graph` if its set of vertices can be divided into two disjoint sets X and Y such that every edge connects a vertex in X to one in Y or vice versa.
...
| Operation/Data Structure | Adjacency Matrix | Adjacency List | Sorted Edge List |
|--------------------------|------------------|----------------|------------------|
| Constructor              | O(\|V\|^2) = O(n^2) | O(\|E\|) = O(m) | O(\|E\|)         |
| Find deg(u)              | O(\|V\|)         | O(1)           | O(\|E\|)         |
| List all adjacent vertices of u | O(\|V\|) | O(deg(u))      | O(\|E\|)         |
| List all edges           | O(\|V\|^2)       | O(\|E\|)       | O(\|E\|)         |
| Check if (u, v) are adjacent | O(1)        | O(deg(u)) (*)  | O(\|E\|)         |
| Smallest edge            | O(\|V\|^2)       | O(\|E\|)       | O(1)             |
| Space                    | O(\|V\|^2)       | O(\|E\|)       | O(\|E\|)         |
